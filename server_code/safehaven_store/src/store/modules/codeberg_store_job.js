...
const codebergHeaders = (env) => {
  const token = (env.CODEBERG_TOKEN || "").trim();
  return {
...
export const normalizeCodebergRepoUrl = (repoUrl) => {
  const clean = (repoUrl || "").toString().trim().replace(/\.git$/, "").replace(/\/$/, "");
  const m = clean.match(/^https?:\/\/codeberg\.org\/([^/]+)\/([^/]+)$/i);
  if (!m) return null;
  return `https://codeberg.org/${m[1]}/${m[2]}`;
};

const parseOwnerRepo = (repoUrl) => {
  const url = normalizeCodebergRepoUrl(repoUrl);
  if (!url) return null;
  const m = url.match(/^https:\/\/codeberg\.org\/([^/]+)\/([^/]+)$/i);
  return m ? { owner: m[1], repo: m[2] } : null;
};

const codebergLatestRelease = async (env, repoUrl) => {
  const parsed = parseOwnerRepo(repoUrl);
  if (!parsed) return null;

  const res = await fetch(
    `${CODEBERG_API}/repos/${parsed.owner}/${parsed.repo}/releases?limit=1&pre-release=false&draft=false`,
    { headers: codebergHeaders(env) }
  );
...
const codebergReleaseAssets = (release) => {
...
const codebergRepoDetails = async (env, repoUrl) => {
  const parsed = parseOwnerRepo(repoUrl);
  if (!parsed) return null;

  const res = await fetch(
    `${CODEBERG_API}/repos/${parsed.owner}/${parsed.repo}`,
    { headers: codebergHeaders(env) }
  );
  ...
  return {
    ...
    repoUrl:     normalizeCodebergRepoUrl(data.html_url || repoUrl) || repoUrl,
  };
};

const codebergReadmeRaw = async (env, repoUrl) => {
  const parsed = parseOwnerRepo(repoUrl);
  if (!parsed) return null;

  const res = await fetch(
    `${CODEBERG_API}/repos/${parsed.owner}/${parsed.repo}/raw/README.md`,
    { headers: codebergHeaders(env) }
  );
...
const getAppByRepoUrl = (env, repoUrl) => {
  const normal = normalizeCodebergRepoUrl(repoUrl);
...
const makePlaceholderPackageName = (repoUrl) => {
  ...
  return `pending.codeberg.${norm(parsed?.owner)}.${norm(parsed?.repo)}`;
};

const displayNameOf = (name, repoUrl) => {
  const fallback = parseOwnerRepo(repoUrl)?.repo || "Unknown App";
  ...
  await env.api_control_db
    .prepare(
      `INSERT INTO store_apps
       ...
       VALUES (?1, ?2, ?3, ?4, ?5, ?6, ?7, ?8, 0, 'unverified', 'active', 0, 1, ?9, ?9, 'codeberg')`
    )
...
const importCandidate = async (env, { repoUrl, adminImport = false }) => {
  const normalized = normalizeCodebergRepoUrl(repoUrl);
  if (!normalized) return { skipped: true, reason: "invalid_codeberg_url" };
  ...
  const release = await codebergLatestRelease(env, normalized);
  if (!release) return { skipped: true, reason: "no_stable_release" };

  const rawAssets = codebergReleaseAssets(release);
  ...
  const details     = await codebergRepoDetails(env, normalized).catch(() => null);
...
export const refreshCodebergMetadataForApp = async (env, app) => {
...
export const pollCodebergApp = async (env, app) => {
  ...
  await refreshCodebergMetadataForApp(env, app);
...
export async function runCodebergDirectImport(env, input = {}) {
  ...
  console.log(JSON.stringify({ tag: "direct_codeberg_import", repoUrl, outcome }));
